...
met_data <-  logger_data[[1]] %>%
  ...
  select(timestamp,air_3m_avg,rh,lgr_ch4_avg,lgr_co2_avg)
...
p2 <- ggplot(met_data) +
  theme_bw() +
  geom_line(aes(x=timestamp,y=lgr_ch4_avg,color="CH4"),size=.1,linetype="twodash")+
  scale_x_datetime()+
  coord_cartesian(ylim = c(1500,2500)) +
  scale_color_manual(values = c(
    "CH4" = "blue")) +
  labs(title = "TWetland Methane",
       x = "Date",
       y = "ppb",
       color = '')+
  theme(plot.title = element_text(hjust = 0.2),
...
p3 <- ggplot(met_data) +
  geom_line(aes(x=timestamp, y=lgr_co2_avg, color = "CO2")) +
  scale_x_datetime()+
  coord_cartesian(ylim = c(300,500)) +
  labs(title ="TWetland Carbon Dioxide",
       x = "Date",
       y = "ppm",
       color = '')+
  scale_color_manual(values = c("CO2" = "red"))+
  theme(plot.title = element_text(hjust = 0.2),
        legend.position = "top",
        axis.title.y = element_markdown(color = "black", size = 8))+
  theme_bw() 
...
min_date <-max(timestamp)- lubridate::days(10)
...
p2_p <- ggplotly(p2,dynamicTicks = T) %>%
        layout(xaxis= xax,
               yaxis = list(autorange=F,range = c(1800, 2300))) %>%
  partial_bundle()
p3_p <- ggplotly(p3,dynamicTicks = T) %>%
  layout(xaxis= xax,
         yaxis = list(autorange=F,range = c(300, 500))) %>%
  partial_bundle()



p <- subplot(p1_p,p2_p,p3_p, nrows=3, shareX = TRUE,titleY = T,heights = c(.2,.4,.4),which_layout = 1)
htmlwidgets::saveWidget(p, "Twetland_CO2_Ch4.html", title = "TWetland CH4 and CO2")
